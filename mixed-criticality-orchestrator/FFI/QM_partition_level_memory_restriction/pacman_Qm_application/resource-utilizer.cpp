...
// CPU worker function - no core pinning
void cpuWorker(int cpuPercent) {
    try {
        ...
        std::cerr << "Error in CPU worker thread: " << e.what() << std::endl;
    }
}
...
void utilizeResources(
    ...
    if (cpuCores > 0) {
        ...
        std::cout << "Starting CPU utilization with " << coresToUse << " threads at " 
                  << cpuPercent << "% CPU load each" << std::endl;
        std::cout << "Threads will automatically be distributed across available CPU cores by the OS" << std::endl;
        
        // Start worker threads - no pinning to specific cores
        for (int i = 0; i < coresToUse; ++i) {
            cpuThreads.emplace_back(cpuWorker, cpuPercent);
        }
    }
