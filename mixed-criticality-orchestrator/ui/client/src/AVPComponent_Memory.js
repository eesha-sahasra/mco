...
const AVPComponent_Memory = forwardRef(({ timePart }, ref) => {
  // State to hold the continuously updated data
  const [data, setData] = useState([]);
  const avpMemory = useSelector((store) => store.avpMemory.avgAvpMemory);

  useEffect(() => {
    // Update the data state to include the new incoming props
    setData((currentData) => [
      ...currentData,
      { time: timePart, avp_mem: avpMemory },
    ]);
  }, [avpMemory, timePart]); // Dependencies array, effect runs when these props change
...
export default AVPComponent_Memory;
