...
const QMPod1 = forwardRef(({ timePart }, ref) => {
  ...
  useEffect(() => {
    const newDataEntry = podUsage[0].reduce((acc, [appName, cpuUsage]) => {
      if (appName === "app1") {
        acc[`${appName}_cpu`] = parseFloat(cpuUsage);
      }
      return acc;
    }, {});
